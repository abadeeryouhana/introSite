...
<div class="card">
    <form action="{{ route('admin.job-positions.store') }}" method="POST">
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:20px; margin-bottom:20px;">
            ...
            <div>
                ...
                <select name="sector" id="sector_select" style="width:100%; padding:10px; border:1px solid #e2e8f0; border-radius:6px; font-size:14px;" onchange="toggleCustomSector(this.value)">
                    ...
                    @endforeach
                    <option value="custom" {{ (old('sector') == 'custom' || (old('sector') && !in_array(old('sector'), $sectors))) ? 'selected' : '' }}>+ Other / Custom Sector...</option>
                </select>
                <div id="custom_sector_wrap" style="margin-top:8px; display:{{ (old('sector') == 'custom' || (old('sector') && !in_array(old('sector'), $sectors))) ? 'block' : 'none' }};">
                    <input type="text" name="custom_sector" id="custom_sector" value="{{ old('custom_sector', (!in_array(old('sector'), $sectors ?? []) ? old('sector') : '')) }}" placeholder="Enter custom sector name..."
                        style="width:100%; padding:10px; border:1px solid #e2e8f0; border-radius:6px; font-size:14px;">
                </div>
            ...
            <div>
                ...
                <input type="text" name="company" value="{{ old('company') }}" placeholder="e.g. Bayan Technology"
                    style="width:100%; padding:10px; border:1px solid #e2e8f0; border-radius:6px; font-size:14px;">
            </div>
            <div>
                <label style="font-weight:600; display:block; margin-bottom:6px;">Location</label>
                <input type="text" name="location" value="{{ old('location') }}" placeholder="e.g. Cairo"
                    style="width:100%; padding:10px; border:1px solid #e2e8f0; border-radius:6px; font-size:14px;">
            </div>
            <div>
        ...
        <div style="margin-bottom:20px;">
            ...
            <textarea name="description" rows="4" placeholder="Short description of the role..."
                style="width:100%; padding:10px; border:1px solid #e2e8f0; border-radius:6px; font-size:14px;">{{ old('description') }}</textarea>
        </div>
